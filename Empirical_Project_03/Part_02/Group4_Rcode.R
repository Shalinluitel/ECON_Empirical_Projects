...
setwd(dir = "C:/Users/Aseni Ariyaratne/Dropbox/Drexel_Econ270/Projects/Project 4/Empirical Project 3/Grading/Group 4")
...
#Open stata data set
proj4 <- read_dta("GROUP_No_4_project3.dta")
head(proj4) 
...
par(mar = rep(2, 4))
table(rank_hat_tree);hist(rank_hat_tree, xlab="Predicted Rates - Single Tree")
...
for (colname in names(proj4)) {
  if (is.character(proj4[[colname]])) {
    proj4[[colname]] <- as.factor(proj4[[colname]])
  }
}

#proj4$predictions_forest <- rank_hat_forest #Add random forest predictions to data set
write.dta(proj4, "Group4_proj4_results.dta") #Save data as a stata .dta file